...
module.exports = {
  ...
  find: async (ctx) => {
    ...
    let contacttag;
    if (ctx.query._q) {
      contacttag = await strapi.query("contacttag", "crm-plugin").search(ctx.query);
    } else {
      contacttag = await strapi.query("contacttag", "crm-plugin").find(ctx.query);
    }

    return contacttag.map((entity) =>
